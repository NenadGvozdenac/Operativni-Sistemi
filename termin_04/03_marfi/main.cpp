...
void kasa(int broj_kupaca, duration<double, milli>& vreme_na_kasi) {
    ...
    std::uniform_real_distribution<float> distribution(0, MAX_MILISEKUNDI_PO_KUPCU);

    for(int i = 0; i < broj_kupaca; i++) {
        int vremeZaCekanje = distribution(gen);
        milliseconds vremeMilisekundi(vremeZaCekanje);
        std::this_thread::sleep_for(vremeMilisekundi);
    }
}
...
int main() {
    ...
    duration<double, milli> najveceVreme = vremena[0];

    for(int i = 1; i < UKUPNO_KASA; i++) {
        if(najveceVreme.count() > vremena[i].count()) {
            najveceVreme = vremena[i];
        }
    }
    ...
    duration<double, milli> duzinaKupcovogIzabira = vremena[moja_kasa];

    if(duzinaKupcovogIzabira.count() > najveceVreme.count()) {
        cout << "Marfijev zakon je potvrdjen." << endl;
    } else {
        cout << "Marfi je omanuo." << endl;
    }
